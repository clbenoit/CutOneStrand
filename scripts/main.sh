...
CONDAPATH=/data/home/jrendu/miniconda3
...
then
	echo -e "python dependencies already available for data preprocessing steps"
	#echo -e  "source activate preprocessdata\n"
else
  echo -e "Installing python dependencies for data preprocessing steps...\n"
  #conda create --name preprocessdata python=3.9 -y
  echo "conda env create --name preprocessdata -f ${WDIR}/environments/preprocessdata.yml"
...
#else
#  echo -e "Installing python dependencies for data preprocessing steps...\n"
#  conda create --name java python=3.9 -y
#  source ${CONDAPATH}/etc/profile.d/conda.sh
#	conda activate java
# conda install -c bioconda picard -y
#	echo "OK"
...
then
	...
	conda create -n pyvcf python=3.9 -y
  source ${CONDAPATH}/etc/profile.d/conda.sh
	conda activate pyvcf
	conda install -c conda-forge pyvcf -y
	conda install biopython numpy pandas -y
	echo "OK"
fi
...
mkdir -p ${WDIR}/annotations/hg38/
cd ${WDIR}/annotations/hg38/
if [ ! -f ${WDIR}/annotations/hg38/gencode.v43.annotation.gtf ];then
  echo -e "Downloading gencode annotation..."
  echo -e "wget https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_43/gencode.v43.annotation.gtf.gz \n"
  wget https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_43/gencode.v43.annotation.gtf.gz
  gunzip ${WDIR}/annotations/hg38/gencode.v43.annotation.gtf.gz
else
...
if [ ! -f ${WDIR}/SNPS/hg38/gnomad.genomes.v3.1.2.sites.${CHR}.vcf.bgz ];then
	...
	cd ${WDIR}/SNPS/hg38/
	echo -e "wget https://storage.googleapis.com/gcp-public-data--gnomad/release/3.1.2/vcf/genomes/gnomad.genomes.v3.1.2.sites.${CHR}.vcf.bgz \n"
...
picard CreateSequenceDictionary R=${WDIR}/sequences/hg38/${CHR}.fa O=${WDIR}/sequences/hg38/${CHR}.dict
echo -e "Getting genomic conctext on candidates positions..."
which java
java --version
...
echo "${WDIR}/SNPS/hg38/${GENE}_SNPS_OFF_TARGETS_hg38.txt"
